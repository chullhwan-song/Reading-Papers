...
> *왜 이 절을 두나*: DMD2 의 6요소 중 가장 단순하지만 가장 결정적 — "안 빼면 SDXL 규모 학습이 사실상 불가능" 이라는 비용 장벽 + "안 빼면 선생 천장을 못 넘는다" 라는 품질 장벽.

#### 3.2.1 그래서 L_reg 가 뭔데?

> *왜 이 소절을 두나*: 비용 분석에 들어가기 전에 "왜 DMD 가 이런 손실을 굳이 끼워 넣었는지" 를 알아야 제거의 의미가 보임.

**한 줄 정의**: **L_reg = 같은 입력 노이즈 z₀ 에 대해, 학생(student) 한 방 출력 G_θ(z₀) 와 선생(teacher)을 50-step 굴려 만든 모범 답안 y_teacher(z₀) 가 픽셀로도 비슷해지도록 강제하는 회귀 (regression) 손실.**

수식 말로 풀면 — 정답 y_teacher(z₀) 와 학생 출력 G_θ(z₀) 의 평균제곱오차 (MSE) 를 더해줌:

```
L_reg = mean( || G_θ(z₀)  −  y_teacher(z₀) ||² )
              학생 출력    선생의 모범 답안
```

DMD 의 본체 학습 신호는 **분포 매칭 그라디언트** (real score − fake score) 였고, L_reg 는 그 위에 얹은 **안전벨트**. DMD 학생 학습 신호 = (분포 매칭) + λ × (L_reg).

#### 3.2.2 DMD 는 왜 안전벨트를 채웠나 — 분포 매칭만 쓸 때의 3가지 약점

> *왜 이 소절을 두나*: 안전벨트가 왜 필요했는지 알아야 DMD2 가 그걸 어떻게 다른 부품으로 대체했는지 이해가 됨.

분포 매칭의 룰은 "**학생이 만든 이미지들의 전체 분포** 가 선생의 분포와 같으면 OK". 분포만 같으면, 같은 z₀ 입력에서 학생이 선생과 **다른 그림**을 그려도 손실 0 이 가능 — 이게 양날의 검:

1. **Mode collapse 위험** — 학생이 분포의 일부 (예: "강아지만") 에 몰려도 그 모드만 잘 학습하면 손실이 0 근처. 분포 매칭은 모드 누락에 민감하지 못함.
2. **High-variance 그라디언트** — `real score − fake score` 는 한 batch 의 두 score 차이의 추정값이라 매 step 방향이 흔들림.
3. **수렴 불안정** — 흔들리는 그라디언트 + 모드 누락 → 학습이 발산하거나 "선생 평균 같은" 무의미한 분포로 수렴.

→ DMD 의 처방: **같은 z₀ 에 대한 정답 y_teacher(z₀) 와 픽셀로도 비슷하라** 는 결정론적 닻 (L_reg) 을 추가. 분포 매칭이 흔들려도 픽셀 닻이 학생을 "선생이 그 입력으로 만든 그 그림" 근처에 묶어 줌.

비유: **그림 모사 수업**. 분포 매칭만이면 "내가 그린 그림들의 *느낌* 이 선생님 그림들의 느낌과 같으면 OK" → 학생이 추상화로 도망갈 수 있음. L_reg 추가하면 "이 사진(z₀) 보고 선생님이 그린 거랑 비슷하게 그려야 해" → 안정적이지만 **모방** 수준.

#### 3.2.3 그런데 안전벨트가 천장이 됨 — DMD2 가 제거한 3가지 이유

> *왜 이 소절을 두나*: 비용만이 아니라 **품질** 측면에서도 L_reg 가 발목을 잡는다는 게 DMD2 의 핵심 주장.

**(1) 편향 (bias)**: L_reg 는 학생을 *선생의 결정론적 출력* 에 묶음. 분포 매칭의 정통 목표 (분포 일치) 와 충돌. 결국 학생이 선생을 그대로 베끼는 수준에 머물러 **선생 천장을 못 넘음** — 이게 본 논문 §4.3 "Surpassing the teacher" 의 핵심 동기.

**(2) 데이터 저장 비용**: L_reg 를 쓰려면 (z₀, y_teacher(z₀)) **쌍 데이터셋** 이 필요. 선생을 미리 한 번 풀어서 저장해둬야 함. SDXL 규모에선 **수백만 장 사전 생성·저장** 이 부담 → 다음 소절의 산수.

**(3) 대체 가능**: 안전벨트의 역할 (mode coverage + 안정성) 을 다른 부품으로 대체 가능
- **Mode coverage** → 진짜 이미지에 직접 GAN loss (§4.3) 로 확보
- **안정성** → critic 의 학습 속도를 늦추는 TTUR (§4.2) 로 확보

→ "**안 빼면 천장**, 빼도 다른 부품이 안전벨트 역할** — L_reg 제거는 단순 단순화가 아닌 **선생을 능가하는 전제**.

#### 3.2.4 게다가 비용까지 — 700 A100 days 산수
...
→ **L_reg 제거 = SDXL 규모 distillation 의 전제**. 품질 (§3.2.3) 만으로도 빼야 하는데, 비용까지 막대 → 빼면 일석이조.
