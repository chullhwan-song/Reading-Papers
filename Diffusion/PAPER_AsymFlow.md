...
**왜 *큰* eigenvalue 방향에 노이즈를 남기나** — *flow matching 의 노이즈→데이터 경로* 가 *데이터 매니폴드 안에서* 일어나야 학습 가치가 있기 때문. 노이즈를 *데이터 매니폴드 방향* 에 정렬시키는 것 (≠ 노이즈의 PCA 가 아님, isotropic 이라 의미 없음).
...
**이중 denoising 의 그림** — *(i) 학습 전 PCA denoising (정답 청소) + (ii) 학습 중 flow matching denoising (네트워크가 노이즈→데이터 경로 학습)*. 두 작용이 서로 직교하므로 *같은 일 반복* 이 아니라 *분담 협력*. 자세한 PCA 절차·시각화는 § 3.5 참조, Fig.2 의 PCA 매핑은 § 3.1 / § 3.2 / § 3.3 참조.
