...
그래서 네트워크 출력 $\hat{u}_A$를 다시 $\hat{u}$로 바꿔야 합니다.
...
|---|---|---|
| AsymFlow $P = AA^T$ | 데이터/노이즈 target | noise 항을 저랭크로 줄임 |
...
3. network output \hat{u}_A를 full velocity \hat{u}로 복원하는 곳은 어디인가?
```
...
|---|---|---|
| (a) Full-rank velocity | $u = \epsilon - x_0$ | PCA denoising을 하지 않은 학습 정답 |
| (a) Full-rank data | $-x_0$ | 이미지 신호. 줄이지 않고 유지 |
| (a) Full-rank noise | $\epsilon$ | 모든 방향에 흩어진 랜덤 노이즈 |
| (a) Asymmetric velocity | $u_A = P\epsilon - x_0$ | bottom noise를 제거한 학습 정답 |
| (a) Low-rank noise | $P\epsilon$ | PCA top-r 방향에만 남긴 노이즈 |
| (b) Low-rank subspace | $P\hat{u}_A$ | top-r 방향. velocity로 그대로 사용 |
| (b) Orthogonal complement | $(I-P)\hat{u}_A$ | bottom 방향. $x_0$ prediction처럼 해석 |
| (b) Final sum | $\hat{u}$ | 두 방향을 합쳐 full velocity 복원 |
