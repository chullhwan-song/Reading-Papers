...
### 3.4 Proportional Attention

토큰 수 L 이 학습 분포 L_train 보다 커지면 softmax logit 의 평균 크기가 변동 → 학습 분포 외 동작. 보정:

```python
# model.py:373-374
softmax_scale = sqrt( log(seqlen, base_seqlen) / head_dim )
```

`base_seqlen = (train_image_size // 16)² = (1024/16)² = 4096`. seqlen 이 같으면 1, 4× 더 크면 √(log_4096(16384)/d) — 자동 보정.

> **학습 시엔 사용 안 함**, 추론 시 `args.proportional_attn=True` 로만 ON.
...
### Q12. 다른 modern T2I (SD3, FLUX) 와의 위치 차이?
