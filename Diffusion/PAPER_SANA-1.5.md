...
| **부분 보존 (Partial Preservation Init, PPI)** ⭐ | 앞 N=20 블록은 그대로, 끝 2 블록 *버리고*, 새 M=40 블록을 가우시안 + 출력 projection 0 | ✅ 채택. 안정적 수렴 |

#### 1.2 PPI의 출력 projection 0 초기화 (= 항등 매핑)
...
논문 명시: *"끝 블록 2개를 드롭하고 새 블록을 덧붙임"*. 추정 이유는 *사전학습된 모델의 끝부분은 latent→diffusion 출력 변환에 특화* 되어 있어 새 블록을 그 *뒤*에 붙이면 이 변환을 두 번 하게 되는 구조적 충돌이 생김. 머리 (1.6B 입력 변환부) 는 보존하고, 꼬리 (1.6B 출력 변환부) 만 새것으로 갈아주는 게 더 자연스러움.
